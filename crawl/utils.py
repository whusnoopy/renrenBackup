...
def get_image(img_url, retry=0):
    ...
    try:
        ...
        logger.fatal(
            "get img {img_url} failed, use blank instead".format(img_url=img_url)
        )
...
def save_user(uid, name, pic=None):
    logger.debug(
        "try to save {uid}[{name}] with headPic {pic}".format(
            uid=uid, name=name, pic=pic
        )
    )
    ...
    logger.debug(
        "saved {uid}[{name}] with headPic {pic}".format(
            uid=uid, name=name, pic=user["headPic"]
        )
    )
    return uid
...
def get_user(uid):
    ...
    try:
        logger.info(
            "    get user {uid} {name} with {pic}".format(uid=uid, name=name, pic=pic)
        )
...
def get_comments(entry_id, entry_type, global_comment=False, owner=crawler.uid):
    ...
    logger.info(
        "        crawled {total}{is_global} comments on {entry_type} {entry_id}".format(
            total=total,
...
def get_likes(entry_id, entry_type, owner=crawler.uid):
    param = {
        ...
        "sourceId": entry_id,
        "owner": owner,
        "gid": "{entry_type}_{entry_id}".format(
    ...
    for like_item in r["likeList"]:
        ...
        like = {"entry_id": entry_id, "entry_type": entry_type, "uid": like_item["id"]}
        Like.insert(**like).on_conflict("replace").execute()

    logger.info(
        "        crawled {count} likes on {entry_type} {entry_id}".format(
            count=len(r["likeList"]), entry_type=entry_type, entry_id=entry_id
        )
    )
    return r["likeCount"]
