...
def load_blog_list(uid=crawler.uid, after=None):
    r = crawler.get_json(
        config.BLOG_LIST_URL, json_=get_common_payload(uid, after), method="POST"
    )
    ...
    for b in r["data"]:
        ...
        try:
            logger.info(
                "  crawled blog {bid} {title} with 评{comment}/分{share}/赞{like}/读{read}".format(
                    bid=bid,
                    title=blog["title"],
            ...
            logger.info(
                "  crawled blog {bid} comment{comment}/share{share}/like{like}/read{read}".format(
                    bid=bid,
                    comment=blog["comment"],
                    share=blog["share"],
                    like=blog["like"],
