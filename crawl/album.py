...
def get_album_summary(album_id, uid=crawler.uid):
    album_data = crawler.get_json(
        config.ALBUM_SUMMARY_URL, json_=get_album_payload(uid, album_id), method="POST"
    )
    ...
    try:
        logger.info(
            "    fetch album {album_id} {name} ({desc}), 评{comment}/分{share}/赞{like}".format(
                album_id=album_id,
                name=album["name"],
                desc=album["desc"],
        ...
        logger.info(
            "    fetch album {album_id}, comment{comment}/share{share}/like{like}".format(
                album_id=album_id,
                comment=album["comment"],
                share=album["share"],
                like=album["like"],
    ...
    for idx, p in enumerate(photo_list):
        ...
        try:
            logger.info(
                "      photo {pid}: {title}, 评{comment}/分{share}/赞{like}/看{view}".format(
                    pid=pid,
            ...
            logger.info(
                "      photo {pid}, comment{comment}/share{share}/like{like}/view{view}".format(
                    pid=pid,
                    comment=photo["comment"],
                    share=photo["share"],
                    like=photo["like"],
...
def get_album_list_page(uid=crawler.uid, after=None):
    albums = crawler.get_json(
        config.ALBUM_LIST_URL, json_=get_common_payload(uid, after), method="POST"
    )
    ...
    for a in albums["data"]:
        ...
        try:
            logger.info(
                "    album {aid}: {name}, has {count} photos".format(
                    aid=aid, name=a["name"], count=a["size"]
                )
            )
        except UnicodeEncodeError:
            logger.info(
                "    album {aid}, has {count} photos".format(aid=aid, count=a["size"])
            )
